...
export default function ScholarshipTestResult() {
    ...
    useEffect(() => {
        if(!user) return
        TrainingService.getScholarshipTestResult(testpaperid, courseid, user.userId,user.firstName+" "+ user.lastName,user.email,user.mobileNo ).then(res => {
            setTestResultData(res);
        })
    }, []);
    const backURL = '/user/app/scholarship/test/' + courseid;
