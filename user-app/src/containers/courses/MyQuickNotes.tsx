          ...
          {subscribedTrainingData.data != null && subscribedTrainingData.data.length > 0 ? (
            <div className="row mt-5">
              {subscribedTrainingData.data.map((item, index) => {
                const sessionURL = `/user/app/training/details/${item.courseId}/${item.id}/0/quicknotes`;
                return (
                  <div className="col-sm-6 mb-3" key={index}>
                    <div className="card">
                      <div className="row m-1">
                        <div className="d-flex flex-row">
                          ...
                          <div className="p-2 w-100">
                            <h5 className="pt-1">{item.course?.name} : Notes</h5>
                      ...
                      <div style={{ fontSize: '12px', paddingTop: '10px' }}>
                        {item.course?.quickNoteCount > 0 && (
                          <span>
                            <i style={{ paddingLeft: '7px' }} className="fa-regular fa-note-sticky"></i>
                            <span style={{ paddingLeft: '5px' }}>
                              <strong>{item.course?.quickNoteCount} Notes</strong>
                            </span>
                          </span>
                        )}
                      </div>
