...
export default function Project() {
  ...
  useEffect(() => {
    ...
    if (CourseType === EnumCourseType.Project) {
      TrainingService.getProjectVideos(courseid, CourseType, user.userId).then(res => {
        setVideoData(res);
        setIsLoading(false);
      });
    } else if (CourseType !== undefined) {
      TrainingService.getSelfPlacedVideos(courseid, EnumCourseType.Project, user.userId).then(res => {
        setVideoData(res);
        setIsLoading(false);
      });
    } else {
  ...
  const bookMarkClick = (contentId: string, courseId: string | any): void => {
    if(!user) return
    const ContentBookMarksDTO: ContentBookMarksDTO = { CourseId: courseId, ContentId: contentId, CourseType: CourseType!, UserId: user.userId };
  ...
  return (
    <div className="mt-4">
      <div className="tab-pane fade show active mt-4 pb-5" id="home" role="tabpanel" aria-labelledby="home-tab">
        ...
        {videoData &&
        videoData.CourseWisePerformance != null && videoData.CourseWisePerformance > 0 && (
          <div className="row">
            <div className="col-sm-4 offset-sm-8 col-xs-4 offset-sm-7">
              ...
              {videoData.TotalLessons != null && videoData.TotalLessons > 0 && (
                <span style={{ fontSize: '12px', paddingLeft: '10px' }}>
                  Completed <strong>{videoData.CompletedLessons}</strong> out of <strong>{videoData.TotalLessons}</strong> Lessons
                </span>
              )}
        ...
        <div className="row pt-3">
          {CourseType !== EnumCourseType.Project && videoData?.courseContentList != null && videoData.courseContentList.length > 0 ? (
            <div>
              {(videoData.IsProjectSubscribed === false || videoData.IsTrialSubscribed === true || videoData.IsMembershipProjectSubscribed === false) && (
                <div className="box-shadow">
                  <div className="p-2">
                    <span className="float-start mt-2 ms-2 mb-3">
                      <strong>Subscribe Project</strong> - Subscribe and get access to Project's videos, code & ppts.
                    </span>
                    <a href={videoData.CourseURL} className="float-end btn btn-primary me-3 mb-1">
                      Subscribe
                    </a>
                  </div>
                </div>
                      ...
                      >
                        <div className="row w-100">
                          ...
                          <div style={{ width: '35%', paddingTop: '5px' }}>
                            <span style={{ float: 'right', fontSize: '13px', fontWeight: '500', paddingRight: '5px' }}>
                            ...
                            <div style={{ padding: '5px' }} key={tIndex}>
                              <div className="row">
                                <div
                                  ...
                                  style={{ color: '#3e3636', fontFamily: 'sans-serif', paddingTop: '5px', paddingLeft: '20px', paddingBottom: '5px' }}
                                >
                                  <h5 style={{ fontSize: '1rem' }}>
                                    <i className="fa-solid fa-list-ul"></i> {topic.TopicName}
                                  </h5>
                                </div>
                              </div>
                              <div>
                                {topic.SubTopics.map((subTopic : any, stIndex) => {
                                  ...
                                  const styleBookMarked = subTopic.IsBookMarked ? cssBookMarked : cssUnBookMarked;
                                  const styleCompleted = subTopic.IsVideoCompleted ? cssCompleted : cssNotCompleted;
                                  return (
                                    <div style={{ padding: '8px', borderBottom: '1px solid #e4ecf2' }} key={stIndex}>
                                      <div className="row">
                                        <div className="col-sm-5 col-xs-12" style={{ paddingTop: '5px', paddingLeft: '20px' }}>
                                          <i
                                            className={subTopic.TopicType === 3 ? 'fa-regular fa-file-lines' : 'fa fa-play-circle icon-player'}
                                            aria-hidden="true"
                                            style= {styleCompleted}
                                          ></i>
                                          <span className="accordion-content__row__title" style={{ fontWeight: '500', fontSize: '14px' }}>
                                        ...
                                        <div className="col-sm-7 col-xs-12" style={{ textAlign: 'right' }}>
                                          <i
                                            ...
                                            className={subTopic.IsBookmarked ? 'fas fa-bookmark' : 'far fa-bookmark'}
                                            onClick={() => bookMarkClick(subTopic.SubTopicId, course.CourseId)}
                                          ...
                                          <VideoActions
                                            ...
                                            selfplaced="project"
                                            membership={videoData.IsMembershipProjectSubscribed}
                                          />
                                        </div>
                                      </div>
                                    </div>
            ...
            <div>{isLoading ? <div className="pt-4"><LoadingSpinner /></div> : CourseType !== EnumCourseType.Project && 'No Project Found!'}</div>
          )}
          {CourseType === EnumCourseType.Project && videoData?.CourseTopics != null && videoData.CourseTopics.length > 0 ? (
            <div>
              {(videoData.IsProjectSubscribed === false || videoData.IsTrialSubscribed === true || videoData.IsMembershipProjectSubscribed === false) && (
                <div className="box-shadow">
                  <div className="p-2">
                              ...
                              <div className="row">
                                ...
                                <div className="col-sm-7 col-xs-12" style={{ textAlign: 'right' }}>
                                  <i
                                    style={styleBookMarked}
                                    className={subTopic.IsBookMarked ? 'fas fa-bookmark' : 'far fa-bookmark'}
                                    onClick={() => bookMarkClick(subTopic.SubTopicId, course.CourseContentId)}
                                    title="Create Bookmark"
                                  ></i>
                                  <VideoActions
                                    subTopic={subTopic}
